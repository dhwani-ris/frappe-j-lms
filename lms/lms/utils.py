    ...
    for chapter in get_chapters(course):
        if get_details:
            ...
            lesson_count += frappe.db.count(
                "Lesson Reference", {"parent": chapter.name}
            )
    for chapter in get_chapters(course):
        if get_details:
            lessons += get_lesson_details(chapter, progress=progress)
...
def has_trainer_role(member=None):
    return frappe.db.get_value(
        "Has Role",
...
def has_master_trainer_role(member=None):
    return frappe.db.get_value(
        "Has Role",
...
def has_lms_manager_role(member=None):
    return frappe.db.get_value(
        "Has Role",
...
def has_lms_hr_role(member=None):
    return frappe.db.get_value(
        "Has Role",
...
def get_batch_details_for_notification(topic):
    ...
    students = frappe.db.get_all(
        "LMS Batch Enrollment", {"batch": topic.reference_docname}, pluck="member"
    )
    users += instructors
    users += students
    return subject, link, users
...
def get_courses(filters=None, start=0):
    ...
    # Jamboree role-based course visibility
    filters = _apply_jamboree_course_visibility(filters)
...
def _apply_jamboree_course_visibility(filters):
    """Restrict course visibility based on Jamboree roles.

    - System Manager / Moderator / HR / Master Trainer / Course Creator: see all
    - Trainer: enrolled + courses they instruct
    - Manager: enrolled + team members' courses
    - Student/Employee: only enrolled courses
    """
    ...
    # Already filtered by enrolled/created — no further restriction
    if (
        "name" in filters
        and isinstance(filters.get("name"), list)
        and filters["name"][0] == "in"
    ):
        return filters

    # Build set of allowed course names
    allowed_courses = set()

    # Always include enrolled courses
    enrolled = frappe.get_all(
        "LMS Enrollment", {"member": frappe.session.user}, pluck="course"
    )
    allowed_courses.update(enrolled)

    is_trainer = "LMS Trainer" in user_roles
    is_manager = "LMS Manager" in user_roles
    ...
    if is_trainer:
        ...
        for c in instructor_courses:
            if frappe.db.exists("LMS Course", c):
                allowed_courses.add(c)

    if is_manager:
        # Team members' enrolled courses
        ...
        if emp_id:
            ...
            if report_user_ids:
                team_courses = frappe.get_all(
                    "LMS Enrollment",
                    {"member": ["in", report_user_ids]},
                    pluck="course",
                )
                allowed_courses.update(team_courses)
...
def get_neighbour_lesson(course, chapter, lesson):
    ...
    chapters = frappe.get_all(
        "Chapter Reference", {"parent": course}, ["idx", "chapter"]
    )
    for chapter in chapters:
        lessons = frappe.get_all(
            "Lesson Reference", {"parent": chapter.chapter}, pluck="idx"
        )
...
def categorize_batches(batches):
    ...
    for batch in batches:
        if not batch.published:
            ...
            getdate(batch.start_date) == getdate()
    ...
    for batch in batches:
        ...
        if frappe.session.user != "Guest":
            if frappe.db.exists(
                ...
                {"member": frappe.session.user, "batch": batch.name},
            ):
...
def get_country_code():
    ...
    try:
        ...
        if data.get("status") != "fail":
            return frappe.db.get_value(
                "Country", {"code": data.get("countryCode")}, "name"
            )
    except Exception:
        pass
    return
...
def get_assignment_details(assessment, member):
    ...
    assessment.url = (
        f"/lms/assignment-submission/{assessment.assessment_name}/{submission_name}"
    )
    assessment.edit_url = f"/assignments/{assessment.assessment_name}"
    submission_name = existing_submission if existing_submission else "new-submission"
    assessment.url = (
...
def get_quiz_details(assessment, member):
    ...
    if len(existing_submission):
        ...
        assessment.completed = True
        assessment.status = (
            assessment.submission.percentage or assessment.submission.score
        )
    else:
        assessment.status = "Not Attempted"
        assessment.color = "red"
...
def get_exercise_details(assessment, member):
    ...
    if frappe.db.exists("LMS Programming Exercise Submission", filters):
        ...
        assessment.completed = True
        assessment.status = assessment.submission.status
        assessment.edit_url = f"/exercises/{assessment.assessment_name}/submission/{assessment.submission.name}"
    else:
        assessment.status = "Not Attempted"
        assessment.color = "red"
        assessment.completed = False
...
def get_batch_students(
    filters, offset=0, limit_start=0, limit_page_length=None, limit=None
):
...
@frappe.whitelist()
def get_batch_students_count(batch):
    """Get count of batch students with permission filtering applied"""
    if not batch:
        return 0

    # Build filters with batch
    enrollment_filters = {"batch": batch}

    # Apply same permission filtering as get_batch_students
    user = frappe.session.user
    roles = frappe.get_roles(user)

    # System Manager, Moderator, Course Creator see all
    if user == "Administrator" or "System Manager" in roles:
        pass  # No filters, see all
    elif "Moderator" in roles or "Course Creator" in roles:
        pass  # No filters, see all
    else:
        # For everyone else, apply employee hierarchy filtering
        employee = frappe.db.get_value("Employee", {"user_id": user}, "name")
        if employee:
            # Get all reporting employees (direct and indirect)
            from lms.lms.custom.batch_enrollment_approval import get_all_reporting_employees
            reporting_employees = get_all_reporting_employees(employee)

            if reporting_employees:
                # User has reports, count their enrollments + own
                ...
                enrollment_filters["member"] = ["in", reporting_users]
            else:
                # No reports, count only own enrollments
                enrollment_filters["member"] = user
        else:
            # Not an employee, count only own enrollments
            enrollment_filters["member"] = user

    return frappe.db.count("LMS Batch Enrollment", enrollment_filters)
...
def get_course_completion_stats(batch):
    ...
    rows = (
        ...
        .on(
            (Enrollment.course == BatchCourse.course)
            & (Enrollment.member == BatchEnrollment.member)
        )
        .where(BatchCourse.parent == batch)
        .groupby(BatchCourse.course, BatchCourse.title)
        .select(
            BatchCourse.title,
            fn.Count(Case().when(Enrollment.progress == 100, Enrollment.member))
            .distinct()
            .as_("completed"),
        )
    ).run(as_dict=True)
...
def get_quiz_pass_stats(batch):
    ...
    rows = (
        ...
        .left_join(Submission)
        .on(
            (Submission.quiz == Assessment.assessment_name)
            & (Submission.member == BatchEnrollment.member)
        )
        .where(
            (Assessment.parent == batch) & (Assessment.assessment_type == "LMS Quiz")
        )
        ...
        .select(
            ...
            fn.Count(
                Case().when(
                    Submission.percentage >= Submission.passing_percentage,
                    Submission.member,
                )
            )
            .distinct()
...
def calculate_course_progress(batch_courses, details):
    ...
    for course in batch_courses:
        progress = (
            frappe.db.get_value(
                "LMS Enrollment",
                {"course": course.course, "member": details.email},
                "progress",
            )
            or 0
        )
...
def calculate_assessment_progress(assessments, details):
    ...
    details.average_assessments_progress = (
        flt((assessments_completed / len(assessments) * 100), 2)
        if len(assessments)
        else 0
    )
    details.average_assessments_progress = (
...
def get_order_summary(doctype, docname, coupon=None, country=None):
    details = (
        get_paid_course_details(docname)
        if doctype == "LMS Course"
        else get_paid_batch_details(docname)
    )
    details = (
        get_paid_course_details(docname)
        if doctype == "LMS Course"
...
def adjust_amount_for_coupon(details, coupon, doctype, docname):
    if not coupon:
        return
    discount_amount, subtotal, coupon_name = apply_coupon(
        doctype, docname, coupon, details.amount
    )
...
def get_payment_doc(payment_name):
    return frappe.db.get_value(
        "LMS Payment",
        payment_name,
        ["name", "coupon", "payment_for_certificate"],
        as_dict=True,
    )
    return frappe.db.get_value(
        "LMS Payment",
...
def update_coupon_redemption(payment_doc):
    if payment_doc.coupon:
        redemption_count = (
            frappe.db.get_value("LMS Coupon", payment_doc.coupon, "redemption_count")
            or 0
        )
    if payment_doc.coupon:
        redemption_count = (
...
def get_programs():
    ...
    published_programs = [
        program for program in published_programs if program not in programs_to_remove
    ]
    programs_to_remove = []
    for program in published_programs:
        if program.name in [p.name for p in enrolled_programs]:
...
def enroll_in_program(program):
    ...
    if not frappe.db.exists(
        "LMS Program Member", {"parent": program, "member": frappe.session.user}
    ):
...
def get_batches(filters=None, start=0, order_by="start_date"):
    ...
    # Jamboree role-based batch visibility
    if isinstance(filters, str):
        filters = frappe.parse_json(filters)
    filters = _apply_jamboree_batch_visibility(filters)
...
def _apply_jamboree_batch_visibility(filters):
    ...
    # Already filtered by enrolled — no further restriction needed
    if (
        "name" in filters
        and isinstance(filters.get("name"), list)
        and filters["name"][0] == "in"
    ...
    # Build set of allowed batch names
    allowed_batches = set()

    # Always include enrolled batches
    enrolled = frappe.get_all(
        "LMS Batch Enrollment", {"member": frappe.session.user}, pluck="batch"
    )
    ...
    if is_trainer:
        # Batches where user is instructor
        instructor_batches = frappe.get_all(
            "Course Instructor", {"instructor": frappe.session.user}, pluck="parent"
        )
        for b in instructor_batches:
            if frappe.db.exists("LMS Batch", b):
                allowed_batches.add(b)

    if is_manager:
        # Team members' batches (direct reports)
        emp_id = frappe.db.get_value(
            "Employee", {"user_id": frappe.session.user, "status": "Active"}, "name"
        )
        if emp_id:
            ...
            report_user_ids = [uid for uid in report_user_ids if uid]
            if report_user_ids:
                team_batches = frappe.get_all(
                    "LMS Batch Enrollment",
                    {"member": ["in", report_user_ids]},
                    pluck="batch",
                )
                allowed_batches.update(team_batches)
